...
description: Bootstrap mechanism for forge when no concept file exists or no existing concept file fits the current session
type: project
---
...
- **Final write**: On READY, forge writes the updated file back to the active path, overwriting the EXPLORING version
...
- **One concept file per repo (fixed path `/docs/concept/concept.md`)** — multiple files supported per repo; a fixed path cannot accommodate multiple concepts
...
**Concept summary**: Forge gains a pre-flight startup sequence that checks `/docs/concept/`, loads all existing files as context, and infers the most relevant one for the current session (confirming with the user). If no file fits or none exist, forge enters bootstrap mode: asks the user where the concept lives, generates an EXPLORING-status concept file using forge format, validates the filename with the user, then runs its standard validation loop. On READY, the final document is written to disk at the confirmed path, overwriting the EXPLORING version. Only the active file is modified; all others remain read-only context.
...
- Initial file status is EXPLORING; READY version overwrites it at session end
